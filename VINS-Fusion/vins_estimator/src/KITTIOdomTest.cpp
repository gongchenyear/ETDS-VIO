...
{
	ros::init(argc, argv, "vins_estimator_drt_dyn");
	ros::NodeHandle n("~");
	ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);

	ros::Publisher pubLeftImage = n.advertise<sensor_msgs::Image>("/leftImage_drt_dyn",1000);
	ros::Publisher pubRightImage = n.advertise<sensor_msgs::Image>("/rightImage_drt_dyn",1000);

	if(argc != 3)
	{
		printf("please intput: rosrun vins_drt_dyn kitti_odom_test_drt_dyn [config file] [data folder] \n"
			   "for example: rosrun vins_drt_dyn kitti_odom_test_drt_dyn "
