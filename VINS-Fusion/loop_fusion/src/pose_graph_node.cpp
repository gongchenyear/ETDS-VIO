...
void image_compressed_callback(const sensor_msgs::CompressedImageConstPtr &image_msg)
{
    ...
    try
    {
        ...
        sensor_msgs::ImagePtr regular_img = boost::make_shared<sensor_msgs::Image>();
        regular_img->header = image_msg->header;
        ptr->toImageMsg(*regular_img);
        
        m_buf.lock();
        image_buf.push(regular_img);
        m_buf.unlock();
...
{
    ros::init(argc, argv, "loop_fusion_dyn");
    ros::NodeHandle n("~");
    ...
    // 判断是否为压缩图像话题 - 通常以"/compressed"结尾
    if(image_topic.length() >= 11 && image_topic.substr(image_topic.length() - 11) == "/compressed")
    {
        // 压缩图像话题 (如 /rgb/image_raw/compressed)
        sub_image = n.subscribe(image_topic, 2000, image_compressed_callback);
        ROS_INFO("Subscribing to compressed image topic: %s", image_topic.c_str());
    }
    else if(image_topic.find("/image_raw") != std::string::npos || 
            image_topic.find("/camera/rgb/image_color") != std::string::npos ||
            image_topic.find("/rgb/image") != std::string::npos)
    {
        // 常见的普通图像话题格式 (如 /rgb/image_raw)
        sub_image = n.subscribe(image_topic, 2000, image_callback);
        ROS_INFO("Subscribing to regular image topic: %s", image_topic.c_str());
    }
    else
    {
        // 其他格式的图像话题
        sub_image = n.subscribe(image_topic, 2000, image_callback);
        ROS_INFO("Subscribing to other format image topic: %s", image_topic.c_str());
    }
